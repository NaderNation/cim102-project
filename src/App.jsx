  ...
  return (
    ...
    >
      ...
      <footer style={{ borderTop: `1px solid ${C.line}`, padding: "28px 24px", textAlign: "center", marginTop: 48 }}>
        ...
        >
          3D visualization disclosure
        </button>
      </footer>

      {modal === "disclosure" && (
        <Modal title="3D Visualization Disclosure" onClose={() => setModal(null)}>
      ...
      {modal === "account" && (
        <Modal title="Create account" onClose={() => setModal(null)}>
          <p style={{ color: C.ink, fontSize: 14, lineHeight: 1.6, margin: "0 0 4px", fontFamily: SANS }}>
            Accounts aren't connected yet. Listings and photos are saved in this browser, but they do not sync to other browsers or devices.
          </p>
        </Modal>
      )}
    </div>
