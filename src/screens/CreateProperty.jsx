  ...
  return (
    <div className="max-w-lg">
      ...
      <div style={{ borderColor: C.line, background: C.paperDim }} className="border rounded-sm p-6">
        ...
        <div style={{ borderColor: C.line, background: "#FBF9F4" }} className="border rounded-sm p-3 mb-4">
          <div style={{ color: C.ink, fontFamily: "Arial, sans-serif" }} className="text-sm font-bold mb-1">
            Import property files
          </div>
          <div style={{ color: C.inkSoft, fontFamily: "Arial, sans-serif" }} className="text-xs mb-3">
          ...
          <button
            ...
            style={{ borderColor: C.brass, color: C.brassDark, fontFamily: "Arial, sans-serif" }}
            className="border rounded-sm px-3 py-2 text-xs font-bold hover:opacity-80"
          >
            Choose files
          </button>
          {importNotice && (
            <div style={{ color: importNotice.includes("could not") || importNotice.includes("No supported") ? "#8C3A2B" : C.forest, fontFamily: "Arial, sans-serif" }} className="text-xs font-semibold mt-2">
              {importNotice}
            </div>
          )}
          {draft.roomSpecs?.length > 0 && (
            <div style={{ color: C.inkSoft, fontFamily: "Arial, sans-serif" }} className="text-[10px] mt-2">
              {draft.roomSpecs.length} room{draft.roomSpecs.length === 1 ? "" : "s"} ready for the floor plan
            </div>
          )}
        </div>
        {error && (
          <div style={{ color: "#8C3A2B", fontFamily: "Arial, sans-serif" }} className="text-xs font-semibold mb-3">
            {error}
          </div>
        )}
        <button
